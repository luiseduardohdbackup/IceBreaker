...
extension MainViewController {
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let advertisementData = device.name.components(separatedBy: separator)
        
        if (advertisementData.count > 1) {
            
            cell.nameLabel?.text = advertisementData[0]
            cell.avatarImageView.image = UIImage(named:  "\(Constants.avatarImagePrefix)\(advertisementData[1])")
